...
class CgTokenBot(refund_sms):
    def __init__(self, business_identifier):
        ...
        super().__init__(f"dev_ci_token_{business_identifier}")
        self.current_step = "Initialization"

    def start(self):
        u_id = self.biz_conf.get("userId", "N/A")
        p_code = self.biz_conf.get("ci", "N/A")

        try:
            self.current_step = "Setup"
            ApiConfig.userid = u_id

            amount = StaticFunctions.get_amount(self.automation_name, Amount.token_ci1, Amount.token_ci2)

            self.current_step = "API Auth (Token Creation)"
            extra = {
                "pageCode": p_code,
                "userId": u_id,
                "description": Descriptions.j4j5,
                "chargeType": ChargeType.j4j5,
                "cField1": Fields.j4j5_ci_1,
                "cField2": Fields.j4j5_ci_2,
                "maxPaymentNum": "6",
                "paymentNum": "1"
            }
            ...
            ci_pre_steps = [
                {"action": "click", "selector": "button.js-credit-btn:has-text('תשלום באשראי')", "iframe": False},
                {"action": "wait", "value": "6"}
            ]

            Fields_and_clicks.run(payment_url, CreditInfo, self, pre_steps=ci_pre_steps)

            self.current_step = "Sync Token Data"
            self.sync_and_save_data(p_code)

            self.current_step = "Audit Suspended (Token Created)"
            audit_errors = run_full_audit(self)
            if audit_errors:
                report = "\n".join(audit_errors)
                update_status_and_check_alert(self, status="FAIL", step="Audit Token",
                                              reason=f"Mismatch: {audit_errors[0]}", audit_report=report)
                raise Exception(f"Audit Mismatch (Token Created): {audit_errors[0]}")
