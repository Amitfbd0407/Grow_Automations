...
class CcPaymentBot(refund_sms):
    def __init__(self, business_identifier):
        ...
        super().__init__(f"dev_cc_{business_identifier}")
        self.current_step = "Initialization"

    def start(self):
        u_id = self.biz_conf.get("userId", "N/A")
        p_code = self.biz_conf.get("cc") or self.biz_conf.get("customer_info", "N/A")

        try:
            self.current_step = "Setup"
            ApiConfig.userid = u_id

            amount = StaticFunctions.get_amount(self.automation_name, Amount.cc1, Amount.cc2)

            self.current_step = "API Auth (Get Payment URL)"
            extra = {
                "pageCode": p_code,
                "userId": u_id,
                "description": f"{Descriptions.cc} - {self.biz_conf.get('name', '')}",
            ...
            self.current_step = "UI Payment (CI Selection Page)"

            ci_pre_steps = [
                {"action": "wait", "value": "3"}
            ]

            Fields_and_clicks.run(
                url=payment_url,
                credit_info=CreditInfo,
                bot=self,
                pre_steps=ci_pre_steps
            )

            self.current_step = "Sync Data"
            self.sync_and_save_data(p_code)

            self.current_step = "API Approve"
            self.approve_transaction(p_code)
            ...
            if audit_errors:
                report = "\n".join(audit_errors)
                update_status_and_check_alert(self, status="FAIL", step="Audit",
                                              reason=f"Mismatch: {audit_errors[0]}", audit_report=report)
                raise Exception(f"Audit Mismatch: {audit_errors[0]}")
            ...
            self.current_step = "Update Standing Order"
            print(f"[Run ID: {self.run_id}] Updating standing order status...")
            standing_order_res = self.update_standing_order(u_id, status=2)

            if standing_order_res and standing_order_res.get("status") != 1:
                print(f"Warning: Standing order update returned status: {standing_order_res.get('status')}")
