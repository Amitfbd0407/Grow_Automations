...
class CgPaymentBot(refund_sms):
    def __init__(self, business_identifier):
        ...
        super().__init__(f"dev_cg_{business_identifier}")
    ...
    def start(self):
        ...
        try:
            ...
            amount = StaticFunctions.get_amount(self.automation_name, Amount.cg1, Amount.cg2)

            self.current_step = "API Auth"
            extra = {
                "pageCode": p_code,
                "userId": u_id,
                "description": Descriptions.cg,
            ...
            self.current_step = "Approve API"
            self.approve_transaction(p_code)

            self.current_step = "Sync Post-Approve"
            self.sync_and_save_data(p_code)
            ...
            if audit_errors:
                report = "\n".join(audit_errors)
                update_status_and_check_alert(self, status="FAIL", step="Audit", reason=f"Mismatch: {audit_errors[0]}",
                                              audit_report=report)
