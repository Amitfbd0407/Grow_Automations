...
class CiPaymentBot(refund_sms):
    def __init__(self, business_identifier):
        ...
        super().__init__(f"dev_ci_{business_identifier}")
        self.current_step = "Initialization"

    def start(self):
        u_id = self.biz_conf.get("userId", "N/A")
        p_code = self.biz_conf.get("ci") or self.biz_conf.get("customer_info", "N/A")

        try:
            self.current_step = "Setup"
            ApiConfig.userid = u_id

            amount = StaticFunctions.get_amount(self.automation_name, Amount.sdk1, Amount.sdk2)

            self.current_step = "API Auth (Get Payment URL)"
            extra = {
                "pageCode": p_code,
                "userId": u_id,
                "description": f"{Descriptions.ci} - {self.biz_conf.get('name', '')}",
            ...
            ci_pre_steps = [
                {"action": "wait", "value": "3"},
                {"action": "click", "selector": ".js-payments button", "iframe": False},
                {"action": "click", "selector": ".js-payments .dropdown-menu span:text-is('1')", "iframe": False},
                {"action": "click", "selector": ".js-credit-btn", "iframe": False},
                {"action": "wait", "value": "6"}
            ]
