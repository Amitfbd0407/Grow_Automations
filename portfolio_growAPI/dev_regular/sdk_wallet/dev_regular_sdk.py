...
class SdkWalletBot(refund_sms):
    def __init__(self, business_identifier):
        ...
        super().__init__(f"dev_sdk_wallet_{business_identifier}")
    ...
    def start(self):
        ...
        try:
            ...
            self.current_step = "API Auth (Get AuthCode)"
            extra = {
                "pageCode": p_code,
                "userId": u_id,
                "description": f"{Descriptions.sdk} Wallet Flow",
            ...
            if not sdk_auth_code:
                ...
                raise Exception(f"API Auth Failed (No AuthCode): {err}")

            self.current_step = "UI Payment (SDK Demo Page)"
